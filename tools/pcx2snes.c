					 ...
					 int num_tiles, int colors)
{
	...
	for(i=0;i<64;i++)
	{
		...
		for(ii=0;ii<num[index];ii++)
			if(combos[index*16+ii]==data) {
				//printf("Color : %i already in block : %i.\n", data, index);
				break;
			}
		...
		if(ii==num[index])
		{
			...
			printf("Add Color : %i in block : %i index : %i combo index : %i\n", data, index, ii, (index*16+ii));
			combos[index*16+ii]=data;
			num[index]++;
			printf("num[Index] is now %i\n",num[index]);
		}
	}
	...
	for(i=0;i<num_tiles;i++)
		if(num[i]==ii) {
			printf("%i colors on tile %i\n",ii,i);
			list[n]=i;
			printf("n : %i list[n] : %i value : %i\n", n, list[n], i);
			n++;
		}
	...
	for(num_final=0;num_final<9;num_final++)
	{
		...
		for(n=index+1; n<num_tiles; n++)
		{
			...
			if(num_miss>=0)
			{
				printf("succeed in combining %i num[test] %i num_miss %i\n",test, num[test], num_miss);
